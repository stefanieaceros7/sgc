...
class Solservicio extends CI_Controller {
    ...
    public function registrarSolServicio() {
        ...
        if ($moveResult) {
            ...
            $valor=str_replace(',', '.', str_replace('.', '', str_replace('$', '', $this->input->post('valor'))));
            $array=explode(".",$valor);
            $valor=$array[0];
    ...
    public function envio($data, $nro_servicio) {
        ...
        $this->email->cc('lsaceros@telebucaramanga.com.co');
            ...
            $texto .= '<tr>
                        <td style="padding: 20px 0 10px 0;
                            ...
                            <a style="background-color: #006dcc;
                               ...
                               padding: 9px 7px;
                               text-decoration: none;" href="' . base_url() . '">Ingresar al sistema</a>
                        </td>
                    </tr>';
        }
        $texto .= '</table>';
        ...
        if ($data['seq_nro_servicio'] != '-1' && $nombreArchivo != null && $nombreArchivo != '') {
            ...
            $file = $path . "/sgc/public/docs/" . $nombreArchivo;
            ...
            if ($data['archivo'] != null && $data['archivo'] != '') {
                $path = $_SERVER["DOCUMENT_ROOT"];
                $file = $path . "/sgc/public/docs/" . $data['archivo'];
                //echo "Antes de adjuntar ";
                $this->email->attach($file);
